...
=== 'Newdc' Token contract with following features ===
    => TRC20 Compliance
...
-------------------------------------------------------------------
 Copyright (c) 2021 onwards Newdc Toke ( https://newdc.live/ )
-------------------------------------------------------------------
*/ 
...
contract Newdc is owned {
    ...
    string constant private _name = "Newdc";
    string constant private _symbol = "NEWD";
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success) {
        ...
        require(
            (_allowance[_from][msg.sender] >= _value) // Check allowance
            && (_balanceOf[_from] >= _value) // Check if the sender has enough
            && (_value > 0) // Don't allow 0value transfer
            && (_to != address(0)) // Prevent transfer to 0x0 address
            && (_balanceOf[_to].add(_value) >= _balanceOf[_to]) // Check for overflows
            && (msg.data.length >= (2 * 32) + 4) //mitigates the TRC20 short address attack
